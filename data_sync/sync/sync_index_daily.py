    ...
    async def sync_incremental(self, start_date: str = None, end_date: str = None, ts_code: str = None):
        ...
        if not ts_code:
            ...
            for code in index_codes:
                try:
                    count = await self._sync_single_index(code, start_date, end_date)
                    total += count
                except Exception as e:
                    self.logger.warning(f"指数 {code} 同步失败: {str(e)}")
            return total
        else:
            return await self._sync_single_index(ts_code, start_date, end_date)
    
    async def _sync_single_index(self, ts_code: str, start_date: str = None, end_date: str = None):
        """同步单个指数"""
        if not start_date:
        ...
        try:
            ...
            if df is None or df.empty:
                self.logger.warning(f"指数 {ts_code} 未获取到数据")
                return 0
            ...
            for i in range(0, len(data_list), self.batch_size):
                ...
                total += count
                self.logger.info(f"指数 {ts_code} 已写入 {total} 条数据")
            
            duration = (datetime.now() - start_time).total_seconds()
            self.logger.info(f"指数 {ts_code} 同步完成，共写入 {total} 条数据，耗时 {duration:.2f} 秒")
            return total
            
        except Exception as e:
            self.logger.error(f"指数 {ts_code} 同步失败: {str(e)}")
            raise
