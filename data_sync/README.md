...
已在 `data_sync/web/` 目录下开发 Web 界面，提供查询和同步功能：
...
1. **首页** (`/`): 显示所有表列表，包含同步按钮和状态指示
2. **浏览数据** (`/table/{table_name}`): 显示表的前100条数据
3. **查看结构** (`/schema/{table_name}`): 显示表的列定义、主键、行数等信息

### 同步 API

1. **触发同步**: `POST /api/sync/{table_name}`
2. **查询状态**: `GET /api/sync/status/{task_id}`
3. **停止任务**: `POST /api/sync/stop/{task_id}`

详细说明见 `data_sync/web/README.md`
