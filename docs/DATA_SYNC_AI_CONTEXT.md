...
├── scripts/                 # 同步脚本集
│   ├── sync_full.py         # 全量同步脚本
│   ├── sync_continuous.py   # 持续同步脚本
│   └── sync_verify.py       # 数据验证脚本
├── models/
│   ├── stock_basic.py
│   ├── stock_daily.py
│   ├── stock_adj_factor.py
...
- `2026-03-22`：`stock_factor_pro` 添加年→月→日三级跳优化逻辑，提升同步效率。
...
## 8.1 stock_factor_pro 年→月→日三级跳优化

`sync_stk_factor_pro.py` 的 `sync_recent_history` 方法实现了三级跳优化逻辑：
...
**核心方法：**
- `_get_year_coverage()`: 查询单年覆盖度
- `_get_month_coverage()`: 查询单月覆盖度
- `_get_missing_trade_dates()`: 精确获取需同步的交易日
